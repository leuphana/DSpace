...
public class DuplicateDetectionTest extends AbstractIntegrationTestWithDatabase {
    ...
    @Test
    public void testSearchDuplicatesWithMultipleFieldsUsingOR() throws Exception {
        // Set the query operator to OR
        configurationService.setProperty("duplicate.comparison.query.operator", "OR");
        // Set configure to use both title and identifier fields, use distance 0 for identifier
        configurationService.setProperty("duplicate.comparison.metadata.field",
            new String[]{"dc.title", "dc.identifier:0"});

        Item item1 = ItemBuilder.createItem(context, col)
            .withTitle("Compare both title or identifier")
            .withIssueDate(item1IssueDate)
            .withAuthor("Surname, F.")
            .withIdentifier("I1")
            .withSubject(item1Subject)
